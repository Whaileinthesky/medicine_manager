...
from PySide6.QtWidgets import QApplication,QPushButton, QListView,QMainWindow
...
class mainwindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.quit.clicked.connect(self.close) #quit으로 종료
        self.ui.Save_Button.clicked.connect(self.save_data) #save로 저장
        #setting list model
        self.medicine_list_model = QStringListModel()
        self.ui.my_medicines.setModel(self.medicine_list_model)

    #텍스트 저장
    def save_data(self,input):
        current_list = self.medicine_list_model.stringList()
        current_list.append(input)
        self.medicine_list_model.setStringList(current_list)
